...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs, float * vec) {
    ...
    const int64_t t_img_a_func_batch_us = ggml_time_us();
    LOG_TEE("Clip A %8.2f ms\n", (t_img_a_func_batch_us - t_img_enc_start_us) / 1000.0);
    {
    ...
    const int64_t t_img_b_func_batch_us = ggml_time_us();
    LOG_TEE("Clip B %8.2f ms\n", (t_img_b_func_batch_us - t_img_enc_start_us) / 1000.0);
    {
    ...
    const int64_t t_img_c_func_batch_us = ggml_time_us();
    LOG_TEE("Clip C %8.2f ms\n", (t_img_c_func_batch_us - t_img_enc_start_us) / 1000.0);
    {
...
#endif
    const int64_t t_img_d_func_batch_us = ggml_time_us();
    LOG_TEE("Clip D %8.2f ms\n", (t_img_d_func_batch_us - t_img_enc_start_us) / 1000.0);
    ggml_backend_graph_compute(ctx->backend, gf);
    
    const int64_t t_img_e_func_batch_us = ggml_time_us();
    LOG_TEE("Clip E %8.2f ms\n", (t_img_e_func_batch_us - t_img_enc_start_us) / 1000.0);
    // the last node is the embedding tensor
    struct ggml_tensor * embeddings = gf->nodes[gf->n_nodes - 1];

    // copy the embeddings to the location passed by the user
    ggml_backend_tensor_get(embeddings, vec, 0, ggml_nbytes(embeddings));
    const int64_t t_img_f_func_batch_us = ggml_time_us();
    LOG_TEE("Clip F %8.2f ms\n", (t_img_f_func_batch_us - t_img_enc_start_us) / 1000.0);
    return true;
}
