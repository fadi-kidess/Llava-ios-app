...
//        self.sampling_ctx.freeSamplingContext()
    ...
    func completion_init(text: String, imageBytes: [UInt8]?) {
        ...
        print("Completion Init A: \(t_heat)")
        if imageBytes != nil {
            ...
            let t_heat_end_B = DispatchTime.now().uptimeNanoseconds
            let t_heat_B = Double(t_heat_end_B - t_start) / NS_PER_S
            print("Completion Init B: \(t_heat_B)")
                  
            print("prashanth: \(success)")
  
        }
        self.sampling_ctx.evaluateString("\(text)\(userPromptPostfix)", batchSize: 2048, addBos: false)
                  let t_heat_end_C = DispatchTime.now().uptimeNanoseconds
                  let t_heat_C = Double(t_heat_end_C - t_start) / NS_PER_S
                  print("Completion Init C: \(t_heat_C)")
    }
